...
- **Rankings (home)** — programs sorted by **team strength** (default), talent, or recruit count. Team strength is 30% talent share + 70% national ranks when both exist, then mixed 75/25 with this season’s opponent strength (SOS). Each school name shows the current-season MaxPreps W–L (e.g. `(8-1)`) when a record is on file — never invented. The Talent cell is a letter grade; the numeric talent total and the grade cutoffs are in the hover/popover. Star columns are labeled 5-star / 4-star / 3-star. The table also shows strength of schedule (mean of this season’s opponents’ prior strength, with a tough/average/light label). Missing SOS is an em dash, not zero. Filters: US state, zip code (≈25-mile haversine). The chevron next to a school name expands **every** 2027+ recruit on that roster (name, position, stars, and spaced 247 · On3 · ESPN · Hudl chips when those URLs exist). The school name still opens `/schools/[id]`.
...
- **Games of the week** — two-sided matchups derived live from MaxPreps **26-27** `schedules.json`, listed by **calendar day** (chronological kickoff date), then by **geometric mean** of home and away Scout talent (`√(home × away)`). Combined talent is still shown on each row and is the within-day tie-break. Each school name shows the current-season MaxPreps W–L (e.g. `(3-0)`) when a record is on file — never invented; missing records are omitted, not shown as 0-0. Played games copy scores from the schedule row (away–home box; never invented). Games missing talent on either side (unmapped / 0) are omitted. State and zip filters use the **game venue** (home school for home games; contest site for neutrals), not either roster’s home state, and keep this same day-then-strength order. The default week window comes from `games-top213.json`.
...
**Team strength** does not invent ranks. It is a two-pass score. Pass 1 (prior): when both `talent_norm` and `ranking_norm` exist, the blend is **30% talent_norm + 70% ranking_norm** (not a 50/50 mean). `talent_norm` is `100 × talent / board-max` (IMG = 100 on talent only). `on3_norm` is `100 × (N + 1 − rank) / N` on the 1000-team On3 national board (rank 1 = 100). `maxpreps_norm` is the same curve on the 100-team MaxPreps **national computer** board (rank 1 = 100, rank 100 = 1). Unranked boards are omitted, never 0. `ranking_norm` is the mean of whichever of On3 and MaxPreps exist. If only talent or only ranking exists, that term is 100% of the prior blend. Texas 6A DCTF Top 25 then adds `10 × (26 − rank) / 25` (#1 +10.00, #25 +0.40) and recent form may move the score by up to ±8. Pass 2: SOS is the mean of this-season MaxPreps opponents’ *prior* (played + remaining). Skip deleted contests, Varsity Opponent, IMG Blue-White, and any opponent with no prior. When SOS exists, `team_strength = 75% × prior + 25% × SOS`; missing SOS leaves prior unchanged — not a penalty. Clamped 0–100. A hard schedule raises the number; a soft one lowers it. IMG is not forced to 100 when MaxPreps has them inside the top 100. Unranked Texas get 0 DCTF extra, not a penalty. Tough/average/light labels are the top/middle/bottom quartile of SOS among schools with at least two known opponents. Game toughness icons compare this team’s final strength to the opponent’s; unmapped opponents are `unknown` (no icon), not cupcakes.
...
python3 scripts/build-strength-and-schedules.py --full-fetch
  # live On3 2026 national composite (1000) + MaxPreps national computer (pages 1–4)
  # + DCTF current 6A Top 25 from texasfootball.com/rankings + MaxPreps 26-27 schedules
  # writes site-data/schedules.json {as_of, season, schools} and site-data/schools-crosswalk.json
  # default restamp (no --full-fetch) still force-refreshes On3 + MaxPreps ranks + DCTF before restamping strength
...
- MaxPreps national computer rankings: https://www.maxpreps.com/football/rankings/{1-4}/ — `__NEXT_DATA__` `rankingsListData` (25/page, `totalCount` 100). Joined by MaxPreps `schoolId` UUID. Unmatched board rows are skipped, never given a made-up school.
- Dave Campbell’s Texas Football 6A Top 25: https://www.texasfootball.com/rankings/ — current Class 6A board. Unmatched names (e.g. a school not in FridayRadar) are skipped, never invented.
